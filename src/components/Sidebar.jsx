...
const Sidebar = ({ setActivePage, activePage }) => {
  const { isAuthenticated, logout } = useContext(AuthContext);  // Get the auth status and logout function
      ...
      <div className="flex-grow px-6 overflow-y-auto">
        ...
        <ul className="space-y-4">
          {menuItems.map((item) => {
            ...
            if (item.label === 'Login' && isAuthenticated) return null; // If user is authenticated, don't show login
            if (item.label !== 'Login' && !isAuthenticated && item.to !== '/login') return null; // If user is not authenticated, don't show other protected pages
            return (
        ...
        <ul className="space-y-4">
          {isAuthenticated && ( // Only show these if the user is authenticated
            <>
              ...
              <li
                className="flex items-center space-x-4 text-gray-100 hover:text-white hover:bg-blue-700 p-2 rounded-lg transition-all transform hover:scale-105 focus:outline-none"
